@extends('layouts.app')
@section('title', 'Right Of Way IA Resolutions')

@section('sidebar')
    <a href="{{ route('row.dashboard') }}">Dashboard</a>
    <a href="{{ route('row.downloadables') }}">Downloadables</a>
    <a href="{{ route('row.resolutions') }}" style="background:rgba(255,255,255,0.1); border-left-color:white;">IA
        Resolutions</a>
@endsection
...
@section('content')
    ...
    <div id="upload-resolution" class="tab-pane">
        <form action="{{ route('row.resolutions.upload') }}" method="POST" enctype="multipart/form-data" data-async-target="#resolutionsList" data-async-reset="true">
            @csrf
